...
/* ── Real Masjid al-Haram photo with 3D parallax tilt ─────────────────────── */
function HaramPhotoCard() {
  ...
  const imgX = useSpring(useTransform(mx, [-0.5, 0.5], [14, -14]), { stiffness: 200, damping: 22 })
  const imgY = useSpring(useTransform(my, [-0.5, 0.5], [14, -14]), { stiffness: 200, damping: 22 })
  ...
  return (
    <div
      className="haram-photo-scene"
      onMouseMove={onMove}
      onMouseLeave={onLeave}
    >
      <motion.div
        className="haram-photo-card"
        style={reduceMotion ? {} : { rotateX: rotX, rotateY: rotY, transformStyle: 'preserve-3d' }}
      >
        {/* ── Main Masjid photo ── */}
        <div className="haram-photo-main overflow-hidden rounded-3xl relative">
          <motion.div
            className="absolute inset-[-8%]"
            style={reduceMotion ? {} : { x: imgX, y: imgY }}
          >
            <SafeImage
              src={hero.src}
              alt={hero.alt}
          ...
          {/* Gradient overlays */}
          <div className="absolute inset-0 bg-gradient-to-t from-night-900/90 via-night-900/20 to-transparent z-10" />
          <div className="absolute inset-0 bg-gradient-to-r from-night-900/30 via-transparent to-transparent z-10" />

          {/* Floating particle stream over the photo */}
          ...
          {/* Card shine */}
          <div className="card-shine pointer-events-none absolute inset-0 z-20" />
        </div>

        {/* ── Floating Medina mini card ── */}
        <motion.div
          className="haram-photo-mini-card"
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-night-900/88 to-transparent" />
          <div className="absolute bottom-2.5 left-2.5 right-2.5">
            <p className="text-[10px] font-semibold text-gold-400 sm:text-xs">{hero.floatingCardLabel}</p>
        ...
        <motion.div
          className="haram-photo-stats-badge glass-card stats-badge-float"
          style={{ transform: 'translateZ(50px)' }}
          initial={{ opacity: 0, x: -20, scale: 0.8 }}
          animate={{ opacity: 1, x: 0, scale: 1 }}
          transition={{ delay: 0.82, duration: 0.6, ease: [0.34, 1.56, 0.64, 1] }}
        >
          <p
      ...
      {/* Ground glow */}
      <div className="haram-photo-glow" aria-hidden="true" />
    </div>
  )
}
...
export default function Hero() {
  ...
  const imageY   = useTransform(scrollY, [0, 700], [0, reduceMotion ? 0 : 80])
  const opacity  = useTransform(scrollY, [0, 450], [1, reduceMotion ? 1 : 0.55])

  const scrollToPackages = () =>
    document.getElementById('packages')?.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth' })
    ...
    >
      ...
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-12 px-4 pt-28 pb-20 sm:px-6 sm:pt-32 lg:grid-cols-2 lg:gap-16 lg:px-8 lg:pb-28">
        ...
        >
          ...
          {/* Animated headline — character split */}
          <motion.h1
          ...
          >
            {hero.title.split(' ').map((word, i) => (
              <span key={i} className={i === 0 ? 'text-gold-gradient' : ''}>
          ...
          >
            <MagneticButton variant="primary" onClick={scrollToPackages} className="w-full sm:w-auto btn-shimmer">
              {hero.primaryCta}
            </MagneticButton>
            <MagneticButton
              variant="outline"
        ...
        {/* ── Right: Real Masjid al-Haram with 3D tilt ── */}
        <motion.div
          className="order-1 mx-auto w-full max-w-md lg:order-2 lg:max-w-none"
          style={{ y: imageY }}
          initial={reduceMotion ? false : { opacity: 0, scale: 0.88, y: 32 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
